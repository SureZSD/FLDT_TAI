...
alpha = 2 # for Dirichlet distribution
...
def separate_data(data, num_clients, num_classes, niid=False, balance=False, partition=None, class_per_client=None):
    ...
    if partition == 'pat':
        ...
        while min_size < least_samples:
            ...
            print("Custom separation")
            for k in range(K):
                idx_k = np.where(dataset_label == k)[0]
                np.random.shuffle(idx_k)
                # proportions = np.random.dirichlet(np.repeat(alpha, num_clients))
                proportions = np.random.dirichlet(np.repeat(alpha, num_clients+2))
                proportions = np.sort(proportions)[-num_clients:]
